...
on the square $x_1 ∈ [−1, 1]$, $x_2 ∈ [−1, 1]$. 

Let's sample 25 points using each of grid-search (circles), random-search (triangles) and latin-hypercube (squares) sampling. 


## Sample designs
\vspace{-1.0cm}
![](images/fig1i){width=30%, fig-align="center"} 

Minimum found by each method shown in grey. 

True global minimum is -1.470 at (0.094, −0.747).
...
## Two important properties of Multivariate Gaussian distributions

If $v_1$ and $v_2$ are two random vectors distributed according to the Multivariate Gaussian distribution, then

1. the joint distribution of $(v_1, v_2)^T$ is also a Multivariate Gaussian,

2. conditional distributions of $v_1 | v_2$ and $v_2 | v_1$ are Multivariate Gaussians.
